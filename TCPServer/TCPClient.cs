    ...
    public class TCPClient
    {
        ...
        */
        ...
        public void InitClient()
        {
            ...
            //! ip와 포트를 넣고 주석 해제
            socket.Connect("10.0.0.15", 52546);
            ...
            if(vecReceive.Count > 0)
            {
                foreach(var msg in vecReceive)
                {
                    ...
                    foreach(var stub in listRmiStub)
                    {
                        ...
                        {
                            Debug.LogError("stub.ProcessReceivedMessage(msg) " + e.Message + "\n" + e.StackTrace + "\n" + msg.jsonBuffer.ToString(Newtonsoft.Json.Formatting.Indented));
                        }
                    }
            ...
            try
            {
                ...
                lock (csSend)
                {
                    if(!isSendComplete)
                    {
                        ...
                        Debug.LogFormat("보낸 데이터: {0} \n 헤더: {1}", packet.jsonBuffer.ToString(Newtonsoft.Json.Formatting.Indented), BitConverter.ToInt32(packet.buffer, 0));
        ...
        private void OnCallbackReceive(IAsyncResult ar)
        {
            try
            {
                ...
                if (readSize > 0)
                {
                   ...
                   */
                    ...
                    while (buffRecvSize >= sizeof(int))
                    {
                        ...
                        Debug.LogFormat("기록된 패킷길이: {0}, 쌓인 버퍼 길이: {1}", packetLength, buffRecvSize);
                        ...
                        if(buffRecvSize >= packetLength)
                        {
                            ...
                            {
                                Debug.LogError("받은 데이터 Json파싱 실패: " + e.ToString() + "\n" + "기록된 패킷길이: " + BitConverter.ToInt32(bufferRecv, 0).ToString());
                                Debug.LogError(Encoding.UTF8.GetString(bufferRecv, BasicType.HEADSIZE, buffRecvSize + packetLength));
                            }
                ...
                {
                    Debug.LogError("OnCallbackRecv() ReadSize == 0 : Socket Close");
                ...
                {
                    Debug.LogError("소켓 연결이 끊겨서 리시브 중단");
                }
            }
            catch(Exception ex)
            {
                Debug.LogError("OnCallbackRecv exception : " + ex.ToString());
                Debug.LogError(Encoding.UTF8.GetString(bufferRecv, 0, bufferRecv.Length));
